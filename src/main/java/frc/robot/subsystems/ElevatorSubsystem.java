...
public class ElevatorSubsystem extends SubsystemBase {
    ...
    private static ElevatorSubsystem extensionSubsystem;

    private static ArmPosition lastExtensionPosition = ArmPosition.Manual; //Travel;

    private double manualSetpoint;

    public static ElevatorSubsystem getInstance() {
        if (extensionSubsystem == null) {
            extensionSubsystem = new ElevatorSubsystem();
        }
        return extensionSubsystem;
    }

    private ElevatorSubsystem() {
        extensionMaster = new TalonFX(11); // SparkMax(50, MotorType.kBrushless);
        extensionFollower = new TalonFX(15); // SparkMax(51, MotorType.kBrushless);
        ...
        extensionFollower.setControl(new Follower(extensionMaster.getDeviceID(), false));
        ;
    ...
    public void setPosition() {
        ...
        double calculation = MathUtil.clamp(pidController.calculate(getExtensionEncoder(), setpoint), -0.1, 0.1);
    ...
    private boolean atLimit(boolean goingDown) {
        ...
        return (goingDown && encoder <= Constants.extensionLowerLimit)
                || (!goingDown && encoder >= Constants.extensionUpperLimit);
    }
